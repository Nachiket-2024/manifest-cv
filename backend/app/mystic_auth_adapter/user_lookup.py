...
async def get_user_id_by_email(email: str, db: AsyncSession) -> int | None:
    ...
    carries email/role/permissions, never an id; see
    current_user_handler.py). ManifestCV feature tables use user_id as
    their foreign key, so every owner-scoped route needs this lookup once
    per request.
    """
