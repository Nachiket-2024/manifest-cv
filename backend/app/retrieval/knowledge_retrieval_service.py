...
async def index_knowledge_base(user_id: int, content: str) -> None:
    ...
    than diffing old vs. new chunks is simpler and avoids stale orphaned
    points from a shrinking section count. Knowledge bases are small enough
    that re-embedding on every save is cheap.
    """
    client = get_client()
