...
3. `Depends(get_current_user)` (mystic-auth's dependency, reached by ManifestCV's own routes through `mystic_auth_adapter` — see [Auth & Authorization](../auth/overview.md)) verifies the JWT and re-queries the user row. For PBAC-gated mystic-auth routes, `Depends(require_authorization(action, resource_type))` additionally resolves the caller's current permissions from their assigned policies. ManifestCV's own routes (career knowledge, resumes, applications) skip PBAC entirely — every result is scoped by `user_id` at the query level instead.
4. On a 401 specifically, `frontend/src/api/setupAuthInterceptor.ts` attempts one silent refresh-and-retry before giving up and marking the session invalid.
