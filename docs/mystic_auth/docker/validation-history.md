...
- Running `.\scripts\docker\dev-up.ps1` reached the attached tail command `docker compose logs -f backend frontend taskiq_worker`. The process was then stopped manually so the validation command did not remain attached.
