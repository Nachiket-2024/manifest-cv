...
| **ManifestCV unit** | `tests/backend/manifestcv/unit/` | `mystic_auth_adapter` — the identity/id-lookup boundary ManifestCV routes depend on |
...
- **A test hangs / can't connect to Postgres from the host**: a native Postgres install or another project's container on the host can still intercept whatever port is configured, even though this stack maps Postgres to the non-default host port `5433` specifically to avoid the common case. See [mystic-auth's own troubleshooting docs](https://github.com/Nachiket-2024/mystic-auth/blob/main/docs/authorization/troubleshooting.md) for the inherited foundation's specific failure modes.
