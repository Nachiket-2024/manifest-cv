...
| Restart policy | `always` on postgres/redis/qdrant/bugsink; app services (backend/frontend/worker) have none — you restart manually | `unless-stopped` on long-running services |
...
scripts/db_restore.sh backups/manifest_cv-20260717-120000.sql
```
...
Starts by default — `docker compose -f docker-compose.prod.yml up -d --build` brings up Bugsink alongside the rest of the production stack automatically, matching upstream's own template default. Unlike the dev compose file, `docker-compose.prod.yml` doesn't run the one-shot `bugsink-seed` step (matches upstream's own prod compose, which stops at starting Bugsink itself) — so `SENTRY_DSN`/`VITE_SENTRY_DSN` need setting manually here: create a project in Bugsink's UI once, then set both env vars to its DSN and redeploy.

`docker-compose.prod.yml` exposes no host port for `bugsink` by default (unlike `backend`/`frontend`, which are meant to be internet-facing) — reaching it in production means an SSH tunnel, a VPN, or a reverse-proxy route you add deliberately. See [Error Monitoring](../../mystic_auth/error-monitoring/overview.md) for the full setup (env vars, DSN wiring — including the internal-vs-browser DSN split that matters even more once the backend isn't on `localhost`, self-hosted Bugsink vs. Sentry's hosted tier).
