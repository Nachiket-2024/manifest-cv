...
**Description**: [Deployment Guide](../deployment/guide.md#backups) documents `scripts/db/db_backup.sh`/`scripts/db/db_restore.sh`, which wrap the `pg_dump`/`psql` commands. They are environment-driven, Docker-only, and cloud-agnostic. What's still missing is a *scheduler*. These scripts still need to be wired into cron, systemd, a managed provider's backup feature, or a sidecar, since no specific production host/cloud target is assumed by this template.
...
**Possible fix**: Add a cron entry, systemd timer, managed Postgres backup, or sidecar container that calls `scripts/db/db_backup.sh` on a schedule. This is provider-specific and left to whoever deploys this.
...
**Description**: [Deployment Guide](../deployment/guide.md#backups) documents backing up/restoring Postgres via `scripts/db/db_backup.sh`/`scripts/db/db_restore.sh`, but Qdrant's vector index (`retrieval/qdrant_client.py`'s `career_knowledge_chunks` collection) has no backup story. It isn't a Docker named volume covered by any documented backup step, and Qdrant itself keeps no history.
...
**Possible fix**: A periodic reconciliation job (compare Qdrant's indexed `user_id` payload values against live `users` rows, delete orphans) is the cleanest fix that doesn't require touching mystic-auth's own code. See [Background Workers](../../mystic_auth/background-workers/taskiq.md) for where a scheduled taskiq task like this would live.
...
**Description**: `docker-build` validates `docker compose config` for dev, local-prod, and prod, checks service dependency references, and builds the backend/frontend images individually. `real-tectonic` runs one real container against real Postgres/Redis. But no CI job runs `docker compose -f docker-compose.prod.yml up` and hits a real endpoint through the actual multi-container wiring (nginx to backend to postgres/redis/qdrant, `depends_on`/healthcheck chains, env var names actually matching between compose `environment:` blocks and what `Settings` expects).

**Impact**: A wiring bug that only manifests when every piece runs together, such as a typo'd env var name, a broken healthcheck dependency, or an nginx `proxy_pass` misconfiguration, could pass every existing CI job and only surface on the first real deployment.
...
**Description**: The `qdrant` service in `docker-compose.yml`, `docker-compose.local-prod.yml`, and `docker-compose.prod.yml` runs with no API key. `retrieval/qdrant_client.py` connects with a bare URL, no credential.
