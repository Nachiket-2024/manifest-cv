...
mystic-auth's own template convention is for the downstream product (`app/`) to own a `sdk.py` (re-exporting the template's building blocks) and an `app_sdk.py` (empty by default, for the product's own re-exports) — see its `docs/mystic_auth/template-usage.md#the-extension-surface-sdkpysdkts-and-app_sdkpyapp_sdkts`. ManifestCV follows this exactly: `backend/app/sdk.py` re-exports everything ManifestCV's own route modules need from mystic-auth, and `backend/app/app_sdk.py` holds ManifestCV's own id-resolution glue. ManifestCV's feature code imports identity logic from exactly these two places, neither of which reaches into mystic-auth's internals directly:
...
No ManifestCV route imports `mystic_auth.auth.current_user`, `mystic_auth.authorization`, `mystic_auth.user_crud`, `mystic_auth.auth.security`, or `mystic_auth.logging` directly — only `app.sdk` (`get_current_user`, `database`, `get_or_404`, `settings`, `capture_exception`, `get_logger`, `rate_limiter_service` — every one of these is a straight re-export sourced from `mystic_auth.*` internals, so ManifestCV's routes take them from `app.sdk` rather than from `mystic_auth.database.connection`/`mystic_auth.api.route_helpers`/`mystic_auth.core.settings`/`mystic_auth.logging.logging_config`/`mystic_auth.auth.security.rate_limiter_service` directly), and `app.app_sdk` for id resolution. Exactly one thing is imported directly instead of through either extension surface: `mystic_auth.database.base` (the declarative `Base` every ManifestCV table model subclasses). This mirrors mystic-auth's own internal modules, which import `Base` the same direct way (e.g. `user_table/user_model.py`) — `sdk.py`'s own `database` export is the session/connection object, not the ORM base class, and a template's own model files were never expected to route through its extension surface for this. `get_logger` and `rate_limiter_service` are both generic, non-identity infrastructure ManifestCV's own routes need (`career_knowledge_routes.py`/`resume_routes.py`/`document_routes.py` use `rate_limiter_service` to throttle their AI/compute-triggering routes — see [Career Knowledge: rate limiting](../career-knowledge/overview.md#rate-limiting)) — both are re-exported by `app.sdk` precisely so reaching for them doesn't mean reaching into mystic-auth's internal module paths. Neither is part of mystic-auth's own upstream `sdk.py` template (its own demo app has no need for them) — they're additions ManifestCV made to its own copy of that file, which is exactly what `sdk.py` is for.
...
`frontend/src/app/career_knowledge/`, `resumes/`, and `applications/` never import from `mystic_auth/auth/`, `mystic_auth/authorization/`, `mystic_auth/store/authStore`, or `mystic_auth/api/axiosInstance`/`mystic_auth/api/apiError` directly — they import shared, non-identity infrastructure (`api`, `queryClient`, `extractApiErrorMessage`, `settings`, plus the shared UI primitives) from `frontend/src/app/sdk.ts`, ManifestCV's own copy of mystic-auth's extension-surface convention, plus mystic-auth's own `mystic_auth/ui/*` primitives re-exported through it (generic UI components with no identity concept) — `frontend/src/app/ui/Pager.tsx` is the one exception: a ManifestCV-added generic pagination component that isn't used by any mystic-auth page, so it lives in ManifestCV's own `app/ui/` rather than mystic-auth's, and is imported directly rather than through `sdk.ts`. ManifestCV's own API client modules (`frontend/src/app/api/application_api.ts`, `resume_api.ts`, `career_knowledge_api.ts`, `document_api.ts`) take the shared axios `api` instance the same way, from `../sdk` (i.e. `app/sdk.ts`), not `../mystic_auth/api/axiosInstance` directly. Session state (is the user logged in at all) is handled once, at the app root, by mystic-auth's own `useAuthSession`/`ProtectedRoute` — ManifestCV's routes are wrapped in the same `ProtectedRoute` (imported from `app/sdk.ts`) as everything else, just without a `permission` prop.
