...
```mermaid
flowchart LR
    subgraph Indexing[On career knowledge save]
        RawInput["raw_input"] --> Structure["structure_knowledge_base()"]
        Structure --> Content["content (Markdown)"]
        Content --> Chunk["chunk_markdown()"]
        Chunk --> Embed1["embed_text() per chunk"]
        Embed1 --> Qdrant[("Qdrant<br/>career_knowledge_chunks")]
    end

    subgraph Generation[On resume generate/refine]
        JobDesc["job_description"] --> Embed2["embed_text()"]
        Embed2 --> Search["search_knowledge_base()"]
        Qdrant --> Search
        Search --> Chunks["matched knowledge_chunks"]
        Chunks --> Generate["generate_resume() /<br/>refine_resume()"]
        Generate --> Resume["resume draft content"]
    end
```
...
Every text-generation call funnels through one shared `_generate_text` helper. A failure (network error, empty response) raises `AIIntegrationError`, which the route layer turns into `502 Bad Gateway` rather than an unhandled `500`, since it's a failure of an upstream dependency.

### Embedding dimensionality

`EMBEDDING_DIMENSIONS = 768`. `gemini-embedding-001` is MRL-trained (Matryoshka Representation Learning), so truncating its native output to a smaller dimensionality via `output_dimensionality` still yields a meaningful vector, unlike naively slicing an untrained model's output. This value must match Qdrant's collection definition exactly (fixed-size vectors).
...
Every outbound Qdrant call (`delete`, `upsert`, `query_points`) goes through `knowledge_retrieval_service._call`, which wraps it in `asyncio.wait_for(..., timeout=15)` and turns any failure (timeout or otherwise) into `retrieval.exceptions.RetrievalError`. Same shape as `ai_integration`'s `AIIntegrationError`/Gemini timeout handling, kept as a separate exception type since it's a different upstream dependency (see that module's own docstring).
...
Re-indexes a user's **entire** knowledge base on every save. It chunks and embeds the current content first, then deletes and upserts that user's points under a per-user Redis lock. If embedding fails, the old Qdrant index remains in place. Full re-indexing is simpler than diffing old vs. new chunks, and a shrinking section count cannot leave stale orphaned points behind. Point ids are deterministic (`uuid5` of `user_id` + chunk index), so re-indexing overwrites in place rather than accumulating duplicates when the chunk count is unchanged.
...
A Gemini failure during structuring/generation/refinement always surfaces as `502 Bad Gateway` (`AIIntegrationError`) at the route level. There's no path where the caller's request succeeds without that content. A Qdrant failure is split by which side of the write it's on:
...
- **Indexing/deletion** (`index_knowledge_base`/`delete_knowledge_base`, called after `career_knowledge_routes.py`'s create/update/delete already committed to Postgres). Best-effort: a `RetrievalError` here is logged and reported to error monitoring (see [Error Monitoring](../../mystic_auth/error-monitoring/overview.md)) but never raised. The Postgres write already succeeded and is the source of truth. Failing the whole request here would incorrectly tell the caller their save failed when it didn't. The tradeoff is that search can go stale until the next successful save. See `career_knowledge_routes.py`'s `_reindex_best_effort`/`_delete_index_best_effort` for the exact reasoning.
