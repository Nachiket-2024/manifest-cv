...
Tailored resume drafts, one per job description a user is applying to. Unlike the career knowledge base (one row per user), a user has many resume drafts. Backend: `resume_table/`, `resume_crud/`, `api/resume_routes/`. Frontend: `frontend/src/app/resumes/`.
