...
`react-router` v8, `BrowserRouter`, defined in `App.tsx`. Only `LoginPage` is eager-loaded (the most common unauthenticated entry point); every other route is `React.lazy`-split. Imports come from the base `react-router` package, not `react-router-dom` — v8 folded the DOM bindings back into the main package, and `react-router-dom` never got a v8 release (its last published version, 7.18.1, carries [GHSA-qwww-vcr4-c8h2](https://github.com/advisories/GHSA-qwww-vcr4-c8h2), fixed only in `react-router@8.3.0`).
